...
func SeedData(db *gorm.DB) {
	if db == nil {
		fmt.Println("Cannot seed data: Database connection is nil")
		return
	}

	var count int64
	if err := db.Model(&models.Joke{}).Count(&count).Error; err != nil {
		fmt.Println("Error counting jokes:", err)
		return
	}
...
<p>The lawyer pulls the drapes, dims the lights and asks in a hushed voice, "How much do you want it to be?"</p>`,
		...
		if err := db.Create(&jokes).Error; err != nil {
			fmt.Println("Error inserting jokes:", err)
		}
	}
}
